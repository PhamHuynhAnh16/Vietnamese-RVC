...
from main.app.core.ui import (
    ...
    change_realtime_preset_choices, 
    ...
    with gr.TabItem(translations["realtime"]):
        ...
        with gr.Row():
            with gr.Accordion(
                ...
                with gr.Row(visible=not client_mode):
                    input_audio_sample_rate = gr.Radio(
                        choices=sample_rate_choice, 
                        value=48000, 
                        label=translations["samplerate"].split("|")[0], 
                        info=translations["samplerate_info"], 
                        interactive=True
                    )
                    output_audio_sample_rate = gr.Radio(
                        choices=sample_rate_choice, 
                        value=48000, 
                        label=translations["samplerate"].split("|")[1], 
                        info=translations["samplerate_info"], 
                        interactive=True
                    )
                    monitor_audio_sample_rate = gr.Radio(
                        choices=sample_rate_choice, 
                        value=48000, 
                        label=translations["samplerate"].split("|")[2], 
                        info=translations["samplerate_info"], 
                        interactive=True,
                        visible=False
                    )
                with gr.Row():
                    asio_output_stereo = gr.Checkbox(
                        label=translations["stereo_device"].split("|")[0], 
                        value=True, 
                        interactive=True,
                        visible=False
                    )
                    asio_monitor_stereo = gr.Checkbox(
                        label=translations["stereo_device"].split("|")[1], 
                        value=True, 
                        interactive=True,
                        visible=False
                    )
            ...
            with gr.Accordion(
                ...
                with gr.Row():
                    presets_name = gr.Dropdown(
                        ...
                        choices=realtime_presets_file, 
                        value=realtime_presets_file[0] if len(realtime_presets_file) > 0 else '', 
                        interactive=True, 
                        allow_custom_value=True
                    )
                with gr.Row():
        ...
        with gr.Row():
            ...
            refresh_preset_button.click(
                fn=change_realtime_preset_choices, 
                inputs=[], 
                outputs=[
                    presets_name
                ]
