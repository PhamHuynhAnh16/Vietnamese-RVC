...
from main.app.core.presets import (
    audio_effect_load_presets, 
    audio_effect_save_presets
)
...
def audio_effects_tab():
    ...
    with gr.Row():
        with gr.Column():
            with gr.Row():
                reverb_check_box = gr.Checkbox(
                    label=translations["reverb"], 
                    value=False, 
                    interactive=True
                )
                chorus_check_box = gr.Checkbox(
                    label=translations["chorus"], 
                    value=False, 
                    interactive=True
                )
                delay_check_box = gr.Checkbox(
                    label=translations["delay"], 
                    value=False, 
                    interactive=True
                )
                phaser_check_box = gr.Checkbox(
                    label=translations["phaser"], 
                    value=False, 
                    interactive=True
                )
                compressor_check_box = gr.Checkbox(
                    label=translations["compressor"], 
                    value=False, 
                    interactive=True
                )
                more_options = gr.Checkbox(
                    label=translations["more_option"], 
                    value=False, 
                    interactive=True
                )    
    with gr.Row():
        with gr.Accordion(
            ...
            with gr.Row():
                upload_audio_files = gr.Files(
                    label=translations["drop_audio"], 
                    file_types=file_types
                )
            with gr.Row():
                audio_in_path = gr.Dropdown(
                ...
                audio_out_path = gr.Textbox(
                    label=translations["output_audio"], 
                    value="audios/audio_effects.wav", 
                    placeholder="audios/audio_effects.wav", 
                    info=translations["provide_output"], 
                    interactive=True
                )
            with gr.Row():
                with gr.Column():
                    audio_combination = gr.Checkbox(
                        label=translations["merge_instruments"], 
                        value=False, 
                        interactive=True
                    )
                    audio_combination_input = gr.Dropdown(
                        label=translations["input_audio"], 
                        value="", 
                        choices=paths_for_files, 
                        info=translations["provide_audio"], 
            ...
            with gr.Row():
                main_volume = gr.Slider(
                    minimum=-80, 
                    maximum=80, 
                    label=translations["main_volume"], 
                    info=translations["main_volume_info"], 
                    value=-4, 
                    step=1, 
                    interactive=True, 
                    visible=False
                )
                combine_volume = gr.Slider(
                    minimum=-80, 
                    maximum=80, 
                    label=translations["combination_volume"], 
                    info=translations["combination_volume_info"], 
                    value=-7, 
                    step=1, 
                    interactive=True, 
                    visible=False
            ...
            with gr.Row():
                audio_effects_refresh = gr.Button(
                    translations["refresh"]
                )
            with gr.Row():
                audio_output_format = gr.Radio(
        ...
        with gr.Accordion(
            ...
            with gr.Row():
                presets_name = gr.Dropdown(
                    ...
                    choices=audio_effect_presets_file, 
                    value=(
                        audio_effect_presets_file[0] 
                    ) if len(audio_effect_presets_file) > 0 else "", 
                    interactive=True, 
                    allow_custom_value=True
                )
            with gr.Row():
            ...
            with gr.Row():
                upload_presets = gr.Files(
                    ...
                    file_types=[".effect.json"]
                )
    with gr.Row():
        apply_effects_button = gr.Button(
            translations["apply"], 
            variant="primary", 
            scale=2
        )
    ...
    with gr.Row():
        ...
        bass_or_treble.change(
            fn=visible, 
            inputs=[
                bass_or_treble
            ], 
            outputs=[
                bass_treble_accordion
            ]
        )
        limiter.change(
            fn=visible, 
    ...
    with gr.Row():
        audio_effects_refresh.click(
            fn=lambda a, b: [
                change_audios_choices(a), 
                change_audios_choices(b)
            ], 
            inputs=[
                audio_in_path, 
    ...
    with gr.Row():
        ...
        refresh_preset_button.click(
            fn=change_effect_preset_choices, 
            inputs=[], 
            outputs=[
                presets_name
            ]
