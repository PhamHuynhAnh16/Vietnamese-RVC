...
def quirk_tab():
    ...
    with gr.Row():
        quirk_choice = gr.Radio(
            label=translations["quirk_label"], 
            info=translations["quirk_label_info"], 
            choices=list(translations["quirk_choice"].keys()), 
            interactive=True, 
            value=list(translations["quirk_choice"].keys())[0]
        )
    with gr.Row():
        apply_quirk_button = gr.Button(
            translations["apply"], 
            variant="primary"
        )
    with gr.Row():
        with gr.Accordion(translations["input_output"], open=False):
            with gr.Row():
                quirk_upload_audio = gr.Files(
                    label=translations["drop_audio"], 
                    file_types=file_types
                )
            with gr.Column():
                quirk_export_format = gr.Radio(
