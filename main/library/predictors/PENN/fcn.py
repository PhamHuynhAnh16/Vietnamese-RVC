...
class FCN(torch.nn.Sequential):
    def __init__(self, channels = 256, pitch_bins = 1440, pooling = (2, 2)):
        super().__init__(*(
            Block(1, channels, 481, pooling), 
            Block(channels, channels // 8, 225, pooling), 
            Block(channels // 8, channels // 8, 97, pooling), 
            Block(channels // 8, channels // 2, 66), 
            Block(channels // 2, channels, 35), 
            Block(channels, channels * 2, 4), torch.nn.Conv1d(channels * 2, pitch_bins, 4)
        ))
